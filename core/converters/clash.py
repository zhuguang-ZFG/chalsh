...
def _convert_vmess(node: Dict[str, Any]) -> Dict[str, Any]:
    proxy = {
        "name": str(node.get("tag", "vmess")),
        "type": "vmess",
        "server": node.get("server", ""),
        "port": int(node.get("server_port", 0)),
        "uuid": node.get("uuid", ""),
...
def _convert_hysteria2(node: Dict[str, Any]) -> Dict[str, Any]:
    proxy = {
        "name": str(node.get("tag", "hysteria2")),
        "type": "hysteria2",
        "server": node.get("server", ""),
        "port": int(node.get("server_port", 0)),
        "password": node.get("password", ""),
...
def _sanitize(proxy: Dict[str, Any]) -> Dict[str, Any]:
    """Remove None values and ensure all values are YAML-safe."""
